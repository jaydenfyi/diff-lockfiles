...
describe('diff (name-grouped resolution pairing)', () => {
  ...
  it('treats a build-metadata-only change as unchanged (semver.eq ignores build)', () => {
    ...
    const oldLock = { packages: { 'foo@1.0.0+build1': makePackage('foo', '1.0.0+build1', 'foo@1.0.0+build1') } };
    const newLock = { packages: { 'foo@1.0.0+build2': makePackage('foo', '1.0.0+build2', 'foo@1.0.0+build2') } };
    expect(diff(oldLock, newLock, false)).toEqual([]);
  });
});
