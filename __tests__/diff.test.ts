...
describe('diff (array output)', () => {
  it('emits one change per package with bare name and provenance keys', () => {
    const oldLock = {
      packages: {
        express: pkg('express', '4.18.0', 'node_modules/express', true), // direct, changed
        accepts: pkg('accepts', '1.3.7', 'node_modules/accepts'), // transitive, changed
      },
      directDependencyInfoAvailable: true,
    };
    const newLock = {
      packages: {
        express: pkg('express', '4.18.2', 'node_modules/express', true),
        accepts: pkg('accepts', '1.3.8', 'node_modules/accepts'),
      },
      directDependencyInfoAvailable: true,
    };
...
describe('diff scope', () => {
  it('marks a paired change direct when either side is direct', () => {
    const oldLock = {
      packages: { lodash: pkg('lodash', '1.0.0', 'lodash@1.0.0', true) },
      directDependencyInfoAvailable: true,
    };
    const newLock = {
      packages: { lodash: pkg('lodash', '2.0.0', 'lodash@2.0.0', false) },
      directDependencyInfoAvailable: true,
    };
...
describe('diff (shallow via directDependencyInfoAvailable)', () => {
  ...
  it('does not filter when shallow but direct info is unavailable (yarn)', () => {
    const oldLock = {
      packages: { lodash: pkg('lodash', '1.0.0') },
      directDependencyInfoAvailable: false,
    };
    const newLock = {
      packages: { lodash: pkg('lodash', '2.0.0') },
      directDependencyInfoAvailable: false,
    };
...
describe('diff (name-grouped resolution pairing)', () => {
  it('pairs one old and one new same-name package as an upgrade even when source keys differ', () => {
    const oldLock = { packages: { 'lodash@4.17.20': pkg('lodash', '4.17.20') } };
    const newLock = { packages: { 'lodash@4.17.21': pkg('lodash', '4.17.21') } };
    expect(diff(oldLock, newLock, false)).toEqual([
  ...
  it('cancels unchanged same-name same-version entries before pairing changes', () => {
    const oldLock = {
      packages: {
        'left-pad@1.1.3': pkg('left-pad', '1.1.3'),
        'left-pad@1.2.0': pkg('left-pad', '1.2.0'),
        'left-pad@1.3.0': pkg('left-pad', '1.3.0'),
      },
    };
    const newLock = {
      packages: {
        'left-pad@1.2.0': pkg('left-pad', '1.2.0'),
        'left-pad@1.3.0': pkg('left-pad', '1.3.0'),
        'left-pad@1.4.0': pkg('left-pad', '1.4.0'),
      },
    };
    expect(diff(oldLock, newLock, false)).toEqual([
  ...
  it('falls back to removed/added for ambiguous many-to-many duplicate replacements', () => {
    ...
    expect(
      diff(oldLock, newLock, false).map((c) => [
        c.kind,
        c.name,
        c.oldVersion?.raw ?? null,
        c.newVersion?.raw ?? null,
      ]),
    ).toEqual([
  ...
  it('reports a single new version as added when no old version matches', () => {
    ...
    const newLock = { packages: { 'foo@1.0.0': pkg('foo', '1.0.0') } };
    expect(diff(oldLock, newLock, false)).toEqual([
  ...
  it('skips the npm root "" project entry (empty name)', () => {
    const oldLock = { packages: { '': pkg('', '1.0.0') } };
