...
async function writeWorkspace(root, pins) {
	...
	for (const [app, version] of pins) {
		...
		await writeFile(
			...
			JSON.stringify(
				{ name: app, version: '1.0.0', dependencies: { 'left-pad': version } },
				null,
				2,
			),
		);
...
async function setupRoot(root, pins) {
	...
	await writeFile(
		...
		JSON.stringify(
			{ name: 'root', version: '1.0.0', private: true, workspaces: ['apps/*'] },
			null,
			2,
		),
	);
...
const ext = (file) =>
	file.endsWith('.json') ? '.json' : file.endsWith('.yaml') ? '.yaml' : '.lock';
