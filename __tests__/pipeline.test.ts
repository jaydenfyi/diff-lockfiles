...
const fixturesDir = join(here, 'fixtures', 'multi-version');
...
describe('pair-by-name bump fix', () => {
  it.each(Object.keys(PAIR_BUMP_FIXTURES))(
    ...
    async (format) => {
      const { filename, old, new: newer } = PAIR_BUMP_FIXTURES[format as keyof typeof PAIR_BUMP_FIXTURES];
      const source = fakeSource(
        { FROM: { [filename]: old }, TO: { [filename]: newer } },
        [filename],
      );
...
const MULTI_VERSION = {
  npm: { filename: 'package-lock.json', ext: '.json' },
  bun: { filename: 'bun.lock', ext: '.lock' },
  pnpm: { filename: 'pnpm-lock.yaml', ext: '.yaml' },
  yarn: { filename: 'yarn.lock', ext: '.lock' },
  aube: { filename: 'aube-lock.yaml', ext: '.yaml' },
} as const;

function loadFixture(manager: string, name: string): string {
  const ext = MULTI_VERSION[manager as keyof typeof MULTI_VERSION].ext;
  return readFileSync(join(fixturesDir, manager, `${name}${ext}`), 'utf8');
}

describe('multi-version resolution (real fixtures)', () => {
  it.each(Object.keys(MULTI_VERSION))(
    'pairs one upgrade for %s after cancelling unchanged versions',
    async (manager) => {
      const { filename } = MULTI_VERSION[manager as keyof typeof MULTI_VERSION];
      const source = fakeSource(
        {
